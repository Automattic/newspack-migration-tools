...
class CraftCMSMigrator implements WpCliCommandInterface {
			...
			foreach ( $bylines_data as $byline_data ) {
				foreach ( $byline_data as $key => $byline ) {
					...
					if ( 1 === count( $exploded_bylines ) ) {
						...
						$wp_user_id = $this->get_or_create_user_by_display_name_or_user_login( $exploded_bylines[0] );
						if ( is_wp_error( $wp_user_id ) ) {
							$this->logger->error( sprintf( "ERROR creating user in post_id %d get_or_create_user_by_display_name_or_user_login for byline '%s' : %s", $post_id, $exploded_bylines[0], $wp_user_id->get_error_message() ) );
							continue;
						}
						...
						foreach ( $exploded_bylines as $exploded_byline ) {
							$wp_user_id = $this->get_or_create_user_by_display_name_or_user_login( $exploded_byline );
							if ( is_wp_error( $wp_user_id ) ) {
								$this->logger->error( sprintf( 'ERROR post ID %d, post_id %d, get_or_create_user_by_display_name_or_user_login for byline %s : %s', $post_id, $exploded_byline, $wp_user_id->get_error_message() ) );
								continue;
							}
	 ...
	 */
	private function get_or_create_user_by_display_name_or_user_login( string $display_name ): int|WP_Error {
		global $wpdb;
		...
		// Get existing user by user_login.
		$user_login = $this->users->get_unused_username( sanitize_user( $display_name, true ) );
		$user_id    = $wpdb->get_var( $wpdb->prepare( 'SELECT ID FROM wp_users WHERE user_login = %s', $user_login ) ); // phpcs:ignore -- WordPress.DB.DirectDatabaseQuery.DirectQuery.
