...
class AttachmentHelper {
	...
	public static function import_attachment_for_post( int $post_id, string $path, string $alt_text = '', array $attachment_args = [], string $desired_filename = '' ): int|WP_Error {
		return self::import_external_file( $path, '', '', '', $alt_text, $post_id, $attachment_args, $desired_filename );
	}
	...
	public static function import_external_file( string $path, string $title = '', string $caption = '', string $description = '', string $alt = '', int $post_id = 0, array $args = [], string $desired_filename = '' ): int|WP_Error {
		...
		if ( ! empty( $maybe_exising_attachment_id ) ) {
	 ...
	 * @return array
	 */
	public static function get_images_sources_from_content( $post_content ) {
		$image_urls = [];
	 ...
	 *
	 * @return array|false See `wp_get_attachment_image_src()`.
	 */
	public static function get_attachment_image_src( int $attachment_id, string $size = 'thumbnail', bool $icon = false ): array|false {
		add_filter( 'jetpack_photon_skip_for_url', '__return_true', 10, 4 );
