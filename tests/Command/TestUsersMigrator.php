...
class TestUsersMigrator extends WP_UnitTestCase {
	...
	public function tearDown(): void {
		// Get all users except the admin user from setUp
		$users = get_users(
			[
				'exclude' => [ $this->admin_user_id ],
			]
		);
		...
		foreach ( $users as $user ) {
			...
			// Delete the user
			wp_delete_user( $user->ID, $this->admin_user_id );
		}

		// Get all posts by admin user
		$posts = get_posts(
			[
				'author'         => $this->admin_user_id,
				'post_type'      => 'post',
		...
		// Restore CAP hook before parent tearDown
		$this->restore_cap_delete_user_action();
